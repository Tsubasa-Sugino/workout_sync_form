...
def _segment_metrics(
    ...
    # 膝評価は「90度に最も近い」フレームで比較する（スクワットと同じ方針）
    _, knee_ref_feat = min(
        items,
    ...
    return {
        ...
        "shoulder_fb_norm": float(knee_ref_feat["shoulder_fb_norm"]),
...
def _deadlift_match_score_percent(template_metrics: dict, target_metrics: dict) -> Dict[str, float]:
    knee_percent = _metric_similarity(
        float(template_metrics["knee_angle_deg"]),
        float(target_metrics["knee_angle_deg"]),
        tolerance=20.0,
    )
    shoulder_percent = _metric_similarity(
        float(template_metrics["shoulder_fb_norm"]),
        float(target_metrics["shoulder_fb_norm"]),
        tolerance=0.25,
    )
    tempo_down_percent = _metric_similarity(
        float(template_metrics["down_time_s"]),
        float(target_metrics["down_time_s"]),
        tolerance=0.7,
    )
    tempo_up_percent = _metric_similarity(
        float(template_metrics["up_time_s"]),
        float(target_metrics["up_time_s"]),
        tolerance=0.7,
    )
...
def _evaluate_manual(
    ...
    if len(tpl_poses) < 2:
        raise ValueError("Template video is too short for manual deadlift comparison.")
    if len(tgt_poses) < 2:
        raise ValueError("Target video is too short for manual deadlift comparison.")
    ...
    score = _deadlift_match_score_percent(tpl_metrics, tgt_metrics)

    template_stem = Path(template_video).stem
    target_stem = Path(target_video).stem
    base_out_dir = os.path.abspath(args.deadlift_compare_out_dir)
...
def evaluate(template_video: str, target_video: str, args: argparse.Namespace) -> None:
    ...
    mode = getattr(args, "deadlift_eval_mode", "auto")
    if mode == "manual":
        _evaluate_manual(
    ...
    matches, all_distances, used_threshold = _search_dtw_matches(
        ...
        target_wave=tgt_pc1,
        step_size=args.deadlift_pca_step_size,
        threshold=args.deadlift_pca_dtw_threshold,
    )
    ...
    print("==== Deadlift compare (PCA waveform + DTW) ====")
    ...
    if len(matches) == 0:
        print("No target deadlift segments were detected by PCA waveform matching.")
        return
    ...
    for i, match in enumerate(matches, start=1):
        ...
        score = _deadlift_match_score_percent(tpl_metrics, tgt)
    ...
    report = {
        ...
        "dtw_config": {
            "step_size": int(args.deadlift_pca_step_size),
            "threshold_input": float(args.deadlift_pca_dtw_threshold),
